...
public class InstructionMessageValidator {
    ...
    private void validateUom(InstructionMessage message) throws ValidationException {
    ...
    private void validateProductCode(InstructionMessage message) {
        if (!isValidProductCode(message.getProductCode())) {
            throw new ValidationException(format(VALIDATION_ERROR_MESSAGE, "product code is not valid Expected:(two uppercase letters before by two digits), Actual: " + message.getProductCode()));
        }
    }
